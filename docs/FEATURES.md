...
`Option::None` becomes NULL (always — typed NAs are the macro `IntoR` path's
convention, and NA input deserializes back to `None`). See
[SERDE_R.md](SERDE_R.md) for details.
